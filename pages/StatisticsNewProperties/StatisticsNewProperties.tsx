...
export const StatisticsNewPropertiesPage: FC = () => {
  ...
  const { isLoading, isError, error, data, refetch } = useQuery<
    ...
    Error
  >({
    queryKey: ['statistics_new_properties', `pageNum.${pageIndex + 1}`],
    queryFn: async () =>
      fetch(`${API_URL}/properties/statistics/new?pageNum=${pageIndex + 1}`, {
        method: 'POST',
        headers: {
  ...
  const sendToWebportal = useMutation({
    mutationFn: async (propertyNumber: string) => {
      return fetch(`${API_URL}/properties/sendtowebportal?propertyNumber=${propertyNumber}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ['statistics_new_properties'] }),
  });

  const newPropertyColumns = useMemo<Array<ColumnDef<StatisticsProperty>>>(
    () => [
      {
        id: 'id',
        accessorKey: 'id',
      ...
      {
        id: 'button',
        header: () => <Heading size='lg'>{translations['releaseToWebportal']}</Heading>,
        cell: ({ row }) => (
          <TableCell>
            <Button
              size='lg'
              ...
              onClick={() => {
                if (row.original.propertyNumber !== undefined) {
                  sendToWebportal.mutate(row.original.propertyNumber);
                }
              }}
            >
  ...
  return (
    <div className={styles.statisticsNewPropertiesPage}>
      <Heading
        size='2xl'
        className={styles.title}
      >
        Statusübersicht der Verbrauchsdaten für Neuanlagen
      </Heading>
